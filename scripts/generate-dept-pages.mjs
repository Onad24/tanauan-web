#!/usr/bin/env node
/**
 * Generates OfficeTemplate-based +page.svelte files for all department pages.
 * Each department gets sensible default data as fallback while Firestore
 * data (managed via /admin/office-page-data) takes priority at runtime.
 */

import { writeFileSync, mkdirSync, existsSync } from 'fs';
import { join } from 'path';

const BASE = 'src/routes/(app)/Departments';

// Per-department default data
const DEPT_DEFAULTS = {
  Accounting: {
...
\tconst defaults = getDeptDefaults('${key}') ?? { department: '${dir}' };
...
for (const [dir, key] of Object.entries(DIR_TO_KEY)) {
  const deptPath = join(BASE, dir);
  const pagePath = join(deptPath, '+page.svelte');

  if (!existsSync(deptPath)) {
    mkdirSync(deptPath, { recursive: true });
  }

  const content = renderPage(dir, key);
  writeFileSync(pagePath, content, 'utf8');
  count++;
  console.log(`✓ ${dir}`);
}

console.log(`\nDone! Generated ${count} department pages.`);
